...
/*
    Skeleton Autonomous Op Mode that holds all initialization and general methods
     All auto op modes should inherit from this
 */
public class DupupodAutoOpMode extends DupupodOpMode{

    //auto start position info
    final Transform AUTO_START_POSTION = new Transform(0.0,0.0,0.0);
    ...
    BotFieldState bot = new BotFieldState(AUTO_START_POSTION.getComponents());
    final double FORWARDS = 1.0;
    final double BACKWARDS = -FORWARDS;
    ...
    private double getDistanceTraveled(){
        ...
        double realityFactor = REAR_WHEEEL_DIAMETER * Math.PI / ANDYMRK_ENC_TICKS;
    ...
    public void turnBotTo(double angle, double finishThreshold) throws InterruptedException {
        ...
        int direction = (int) getSign(angle - startAngle);
        driveSmallWheels(-0.88*direction,1.0*direction);
